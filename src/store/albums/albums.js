...
import { updatePhoto as updatePhotoMutation } from '@/graphql/mutations';
...
export const albumInfo = {
  ...
  actions: {
    ...
    async updatePhoto({ dispatch }, updatedPhoto) {
      try {
        await API.graphql(
          graphqlOperation(updatePhotoMutation, { input: updatedPhoto })
        );
        dispatch('getAlbumsData');
      } catch (error) {
